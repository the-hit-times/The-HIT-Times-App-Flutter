...
class LiveMatchRepo {
  ...
  LiveMatchRepo() {
    _liveSessionRef = _db
        .collection(_liveSessionCollection)
        .where(LiveMatch.FIELD_MATCH_TYPE, isEqualTo: "football")
        .withConverter(
            fromFirestore: LiveMatch.fromFirestore,
            toFirestore: (LiveMatch liveMatch, options) =>
                liveMatch.toFirestore());
  }

  getCrickLiveMatches() {
    return _db
        .collection(_liveSessionCollection)
        .where(LiveMatch.FIELD_MATCH_TYPE, isEqualTo: "cricket")
        .withConverter(
            fromFirestore: LiveMatch.fromFirestore,
            toFirestore: (LiveMatch liveMatch, options) =>
                liveMatch.toFirestore())
